...
extern "C" void arm_FFT(float32_t data[]);
...
namespace siege{
	...
	public:
		...
		void siggenf(u16 amp)
		{
			for(u16 i=0;i<1024;i++)
			{
				dataf[i] =  +20*sin(2*3.1415*2*i*90/(1024)+0) ;//+ 60*rand()/32768.0;
				dataf[i] +=  +60*sin(2*3.1415*2*i*amp/(1024*10)+0) ;//+ 60*rand()/32768.0;								
			}
		}
		void scale_fft(void)
		{
		...
		void fscale_fft(void)
		{
			float maxval = 0;
			i16 dessize = 150;
			for(u16 i =0;i<1024;i++)
			{
				if(dataf[i]>maxval) maxval = dataf[i];
			}
			float scalefact = dessize/(float)maxval;
			for(u16 i =0;i<1024;i++)
			{
				dataf[i] = (dataf[i] * scalefact)+dessize/2;
			}
			...
			:mode(1),pause(0)
		{
			...
			widgets::CGraphWidget *gw,*gw2,*gw3;
			...
			gw3 = new widgets::CGraphWidget(surf,220+400+20,20,600,300+20,1);
			gw3->pData = dataf;
			gw3->size = 1024;
			gw3->plotcolor = 0xffffffff;
