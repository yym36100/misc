		...
		void fft(void)
			{
				...
				for(u16 i=0;i<1024*1;i++)
				{
					...
					f[i] = (short)gw1->sig->pdata[i];
				}
				scale = fix_fftr(f,10,0);
				float *ff = new float[1024];
				for(u16 i=0;i<1024;i++)
				{
					ff[i]= -(f[i]*f[i]);//+f[512+i]*f[512+i]);
					//ff[i]= -f[i];
				}
				//float *fg = new float[1024];
				//for(u16 i=0;i<256;i++)
				//{
				//	fg[i*2]= ff[i];
				//	//fg[i+1]= ff[i+1];
				...
				//	
				//}
				((widgets::CGraphWidget*)(gw1->next))->sig = new widgets::CSignal(0,0,0,0,1024/1,siege::widgets::CSignal::enext,ff);
				((widgets::CGraphWidget*)(gw1->next->next))->sig = new widgets::CSignal(0,0,0,0,1024/1,siege::widgets::CSignal::enext,ff);
				//delete[] ff;
