...
VOID FillBuffer (PINT pBuffer, int iFreq)
{
	static double fAngle ;
	int           i ;	
	for (i = 0 ; i < OUT_BUFFER_SIZE ; i++){
		fAngle += 2 * 3.14159265358 * iFreq  / 11000;			
	...
	case WM_COMMAND:
		...
		case IDC_ONOFF:
			...
			if (hWaveOut == NULL)
			{
				...
				pBuffer1  = malloc (OUT_BUFFER_SIZE*2) ;
				pBuffer2  = malloc (OUT_BUFFER_SIZE*2) ;
