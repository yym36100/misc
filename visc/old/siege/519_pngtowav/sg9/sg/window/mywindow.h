...
namespace sg{
	...
	public:
		...
		CWindow( r, style, parent_hWnd, name),w(0)
		{
			...
			w2=(new wg::SpectroGraph(surf,10,200+2+10+10+10,1024+2,480+2,512,"Spectrum"));		
			((wg::CGraphWidget*)w2)->style  = wg::CGraphWidget::filled;

			w->Add(w2f);
			w->Add(w2);

			ws = new wg::CSlider(surf,10+30,600+22+100+2+8+5,1024+2-100);
			w->Add(ws);

			wt = new wg::CTextWidget(surf,1024-25,600+22+100+2+8+5,8*5,8,"00:00",gray);
			w->Add(wt);

			wb = new wg::CButton(surf,10,600+22+100+2+8,20,20);
			wb->SetText((u8*)"P");
			w->Add(wb);
