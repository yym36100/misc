...
namespace siege
{
	namespace widgets
	{
				...
				case ensin:
					{
						...
						for(int i = 0;i<12;i++)
						{
							aa[i] = a+ a*sin(2*3.141592*i/12.0 + f/512.0*2*3.141592*8);
							aa[i] /= 2;
						}
						...
						for(u16 i=0;i<length;i++) 
						{
							...
							pdata[i] = 0;
							pdata[i] += offs + aa[0] * sin(2*3.141592 * (f+0*s)*i / (float) length + ph);
							pdata[i] += offs + aa[1] * sin(2*3.141592 * (f+1*s)*i / (float) length + ph);
							pdata[i] += offs + aa[2] * sin(2*3.141592 * (f+2*s)*i / (float) length + ph);
							pdata[i] += offs + aa[3] * sin(2*3.141592 * (f+3*s)*i / (float) length + ph);
							pdata[i] += offs + aa[4] * sin(2*3.141592 * (f+4*s)*i / (float) length + ph);
							pdata[i] += offs + aa[5] * sin(2*3.141592 * (f+5*s)*i / (float) length + ph);
							pdata[i] += offs + aa[6] * sin(2*3.141592 * (f+6*s)*i / (float) length + ph);
							pdata[i] += offs + aa[7] * sin(2*3.141592 * (f+7*s)*i / (float) length + ph);
							pdata[i] += offs + aa[8] * sin(2*3.141592 * (f+8*s)*i / (float) length + ph);
							pdata[i] += offs + aa[9] * sin(2*3.141592 * (f+9*s)*i / (float) length + ph);
							pdata[i] += offs + aa[10] * sin(2*3.141592 * (f+10*s)*i / (float) length + ph);
							pdata[i] += offs + aa[11] * sin(2*3.141592 * (f+11*s)*i / (float) length + ph);


							/*pdata[i] += offs + a/2 * sin(2*3.141592 * f2*i / (float) length + ph);
							pdata[i] += offs + a/2 * sin(2*3.141592 * (f2+10*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2+20*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2+30*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2+40*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2+50*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2+60*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2-60*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2-50*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2-40*2)*i / (float) length + ph);
							pdata[i] += offs + a /2* sin(2*3.141592 * (f2-30*2)*i / (float) length + ph);
							...
							//						pdata[i] += offs + (a/0.0625) * (rand()-32767.0/2.0)/32768.0;
		...
		public:
			...
			sig(0),surf(ss),CWidget(x,y,w,h), dmode(enstep), autoscale(true),peakdetect(true), style(enplot), clip(0),palette(0),yscale(0)
			{
				title[0]=0;
			...
			void DrawSignal(void) 
			{
				...
				{
					char buffer[200];
					sprintf(buffer,"%s m:%2.2f M:%2.2f fact:%2.2f ofs:%2.2f",title, m,M,fact,ofs);
					sf->Color = 0xff808080;
					sf->DrawTextB(rect.x+2, rect.y+2,(u8*)buffer);
				}
			}

			void DrawSignal2(void) 
			{
				if(!sig) return;
				float j = sig->length/(float)rect.w;
				if(clip!=0)
				...
				for(u16 i=0;i<rect.w;i++)
				{
					...
					{
						float avg = 0;
						for(u8 n=0;n<(u8)j+1;n++)
						{
							avg += pdata[(u16)ind+n]/fact;
						}
						avg/= (double)(j+1);
						val = ofs + avg;
					}
