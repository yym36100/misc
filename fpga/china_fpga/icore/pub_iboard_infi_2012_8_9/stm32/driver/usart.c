...
static int initialize_usart3(int baudrate)
{
	...
	USART3->BRR.B.DIV_Fraction = temp; //DIV_Fraction[3:0]：USARTDIV的整数部分
	...
	//113 115200
	USART3->CR1.B.M = 0;              //0：一个起始位，8个数据位，n个停止位；
	USART3->CR2.B.STOP = 0;   //00：1个停止位；
	USART3->CR1.B.PCE = 0;    //0：校验控制被禁止；
	USART3->CR3.B.RTSE = 0;   //0：RTS硬件流控制被禁止；
	USART3->CR3.B.CTSE = 0;   //0：CTS硬件流控制被禁止；
	USART3->CR1.B.TE = 1;             //1：发送被使能。
	USART3->CR1.B.RE = 1;             //1：接受被使能。
	USART3->CR2.B.CLKEN = 0;  //1：SCLK引脚被使能。
	USART3->CR2.B.CPOL = 0;   //0：总线空闲时SCLK引脚上保持低电平；
	USART3->CR2.B.CPHA = 1;   //1：时钟第二个边沿进行数据捕获。
	USART3->CR2.B.LBCL = 0;   //0：最后一位数据的时钟脉冲不从SCLK输出；


	USART3->CR1.B.PEIE = 1;   //1：当USART_SR中的PE为1时，产生USART中断。
	USART3->CR1.B.RXNEIE = 1; //1：当USART_SR中的ORE或者RXNE为1时，产生USART中断。

	USART3->CR1.B.UE = 1;             //1：USART模块使能。
